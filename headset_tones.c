...
typedef enum {
	WAV_RING_TONE_BASE = 0xA0 ,
	
	ring_wav_01 = 0xA0 ,/* powerup 8k 8bit wav ringtone */
	ring_wav_02 = 0xA1 , /* powerup 8k 8bit wav ringtone */
...
{    
    uint16 lEvent = pEvent - EVENTS_MESSAGE_BASE ;
	int i = 0 ;
	
    /* If there's a valid TTS event to play don't play any tones */
    if(TTSPlayEvent( pEvent ))
        return;
    
    /* scan available tones list, list end is signified by NOT_DEFINED */
	while ( theHeadset.audioData.gEventTones [i].tone != TONE_NOT_DEFINED )
	{
        /* if an event matche is found then play tone */
		if (theHeadset.audioData.gEventTones [i].event == lEvent )
		{
			...
			switch(pEvent)
            {
                ...
                case TONE_TYPE_RING_2:
       	            ...
       	            TonesPlayTone (theHeadset.audioData.gEventTones [ i ].tone ,FALSE, (theHeadset.features.MuteToneFixedVolume)) ;			
                    break;
                ...
                default:
					...
					if((theHeadset.TTS_ASR_Playing == false))/*EventLowBattery*/
					{
						if(lEvent == 0x1A)
							TonesPlayTone (theHeadset.audioData.gEventTones [ i ].tone ,theHeadset.features.QueueEventTones, FALSE ) ;
					}
					#else
    				/* play tone */
            	    TonesPlayTone (theHeadset.audioData.gEventTones [ i ].tone ,theHeadset.features.QueueEventTones, FALSE ) ;			
...
DESCRIPTION
  	Playsback the tone given by the headsetTone_t index
...
void TonesPlayTone ( uint16 pTone , bool pCanQueue , bool PlayToneAtDefaultLevel)
{
    ...
    if (pTone < WAV_RING_TONE_BASE) 
    {
        ...
        else
        {
            ...
            if(&theHeadset.audioData.gConfigTones->gVariableTones[0] && 
               theHeadset.audioData.gConfigTones->gVariableTones[pTone-1-NUM_FIXED_TONES])
            {
                ...
                AudioPlayTone ( (const ringtone_note *)(&theHeadset.audioData.gConfigTones->gVariableTones[0] + 
                                                     (uint16)theHeadset.audioData.gConfigTones->gVariableTones[pTone-1-NUM_FIXED_TONES]),
                                pCanQueue,
                                theHeadset.codec_task,
                                lToneVolume,
                                theHeadset.features.stereo ) ;  
...
#ifdef WAV_RINGTONES
    else if(pTone < WAV_RING_TONE_TOP) 
    {        
        TONE_DEBUG(("TONE Play wav [%x]\n", pTone));
        AudioConnect ( (TaskData *) &csr_wav_ringtone_plugin,
                       0, 
                       0,
                       theHeadset.codec_task,
                       lToneVolume,
                       ...
                       RINGTONE_MODE_CONNECTED, 
                       RINGTONE_ROUTE_INTERNAL,
                       LBIPMPowerLevel(),
                       NULL,
                       NULL) ;

        AudioPlayTone(	(const ringtone_note *)(pTone - WAV_RING_TONE_BASE),
                        pCanQueue,
                        theHeadset.codec_task,
                        lToneVolume,
                        theHeadset.features.stereo);
    ...
    {
        DEBUG(("TONE Out of range tone [%x]\n", pTone));
        Panic();
    }
